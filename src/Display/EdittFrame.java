...
public class EdittFrame extends JFrame {
	int indexUser;
	List_BookIn b;
    String id;
    //hàm dựng khởi tạo nhận vào thông tin về id và số thứ tự người dùng để quay lại giao diện trước đó
   public EdittFrame(int indexUser,String id) {
	   b=new List_BookIn();
 ...
 //search sách theo id rồi gán giá trị vào bảng
   public void SetData() {
	ArrayList<BookIn> lst=b.getAllBookIn();	
	for(BookIn bookin:lst){
		if(bookin.getId().equals(id)) {
			idTextField.setText(bookin.getId());
			nameTextField.setText(bookin.getName());
			authorTextField.setText(bookin.getAuthor());
			publisherTextField.setText(bookin.getPublisher());
			
			int month=bookin.getYear().getMonth()+1;
			int day=bookin.getYear().getDate();
			int year=bookin.getYear().getYear()+1900;
			...
			numberSpinner.setValue(bookin.getNumber());
			costSpinner.setValue(bookin.getCost());
			switch (bookin.getCategory()) {
			case "lịch sử":
				lichsuRadioButton.setSelected(true);
				break;
			case "pháp Luật":
				phapluatRadioButton.setSelected(true);
				break;
			case "tâm lí":
				tamliRadioButton.setSelected(true);
				break;
			case "văn học":
				vanhocRadioButton.setSelected(true);
				break;
			case "ngôn tình":
				ngontinhRadioButton.setSelected(true);
				break;
			case "thiếu nhi":
				thieunhiRadioButton.setSelected(true);
				break;
			case "kinh tế":
				kinhteRadioButton.setSelected(true);
				break;
			case "giáo trình":
				giaotrinhRadioButton.setSelected(true);
				break;
			default:
				break;
			}
    ...
    //tại các giá trị trong comboBox
     public void addYear(){
    ...
    //ấn back quay về màn hình admin
